...
MarketPanelは、古いAndroidタブレットを株価・ETF・国内投信・市場指標の常時表示モニターとして再利用するためのアプリです。

米国株・ETF、国内投信の実基準価額、市場指数・為替を3つのパネルに表示し、設定した間隔で自動的にローテーションします。実データはCloudflare Workerを経由してTwelve Data、Yahoo!ファイナンス系データ、Cboe公式履歴から取得します。
...
- 3つの表示パネル
  - `MAIN`: 米国株・米国ETF
  - `MAIN2`: 日本株・国内ETF（上段MAINを米国株と共有）
  - `SUB1`: 国内投信の実基準価額
  - `SUB2`: 市場指数・為替
- 銘柄の追加、編集、削除、並べ替え、有効・無効設定
- 米国株と日本株を交互に表示しながら上段の大きなチャートを維持
- 5秒（デバッグ用）から5分までの表示ローテーション
- 通常時はMAIN、SUB1、SUB2の切り替えを10秒ずつずらして表示
- 現在値と週足チャートの端末内Roomキャッシュ
- 国内投信5本の円建て基準価額・前日比・基準日・過去1年の週次グラフ
- 日本株・国内ETFと市場指標の過去1年の週次グラフ（VIXを含む）
- Cloudflare CacheとKVによるサーバー側キャッシュ
- 日本時間の市場スケジュールに合わせた終値中心のデータ取得（標準）
- 終値中心、日本株日中更新、4時間更新、デバッグの取得モード
...
- 指定時刻に黒画面・最低輝度へ移行し、通信とローテーションを止める夜間モード
